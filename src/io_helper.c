...
{
    //if (cfg->n_tenants == 0)
    if (sigshared_cfg->n_tenants == 0)
        return 0;

    //int result = cfg->tenants[0].weight;
    int result = sigshared_cfg->tenants[0].weight;

    //for (int i = 1; i < cfg->n_tenants; i++)
    for (int i = 1; i < sigshared_cfg->n_tenants; i++)
    {
        //result = gcd(result, cfg->tenants[i].weight);
        result = gcd(result, sigshared_cfg->tenants[i].weight);
    }
...
int get_max_weight(void)
{
    ...
    int max_val = sigshared_cfg->tenants[0].weight;

    //for (int i = 1; i < cfg->n_tenants; i++)
    for (int i = 1; i < sigshared_cfg->n_tenants; i++)
    {
        //if (cfg->tenants[i].weight > max_val)
        if (sigshared_cfg->tenants[i].weight > max_val)
        {
            //max_val = cfg->tenants[i].weight;
            max_val = sigshared_cfg->tenants[i].weight;
        }
    }
...
int init_tenant_pipes(void)
{
    ...
    int num_tenants = sigshared_cfg->n_tenants;
    ...
    for (int i = 0; i < num_tenants; i++)
    {
        ...
        tenant_pipes[i].weight = sigshared_cfg->tenants[i].weight;
...
int add_weighted_pipes_to_epoll(int epoll_fd, struct epoll_event *ev)
{
    ...
    for (int i = 0; i < sigshared_cfg->n_tenants; i++)
    {
