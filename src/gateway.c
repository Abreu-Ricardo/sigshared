...
uint64_t get_program_ns(){
    ...
    // volta o ponteiro para o comeco do arquivo
    rewind(f_bpf);
    return r_ns;
}
/***********************************************************************/
...
static void metrics_collect(void){
    ...
    //printf("Entrou no metrics_collect()\n");
...
static int gateway(void)
{
    ...
    if(sprintf(path_stats, "/proc/%d/fdinfo/%d", pid_gateway, prog_fd) < 0){
        perror("Erro ao criar o caminho para pid_gateway");
        exit(1);
    }

    f_bpf = fopen(path_stats, "r");
    if(f_bpf == NULL){
        perror("Erro ao abrir o arquivo de stats");
        exit(1);
    }
